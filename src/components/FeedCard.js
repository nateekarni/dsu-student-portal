...
import { formatDateRange, getTimeAgo, formatLocation, formatCurrency } from '../utils.js';

/**
 * FeedCard Component - แสดงโครงการในรูปแบบ Feed
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition">
            ...
            <div class="px-4 pb-3">
                ...
                <div class="flex flex-wrap gap-2">
                    ${location ? Tag({ icon: 'fa-location-dot', iconColor: 'text-red-400', text: location }) : ''}
                    ${Tag({ icon: 'fa-users', iconColor: 'text-blue-400', text: `รับ ${quota || '-'} คน` })}
                    ${Tag({ icon: 'fa-calendar', iconColor: 'text-orange-400', text: dateRange, iconType: 'regular' })}
                </div>
            </div>
            ...
            <div class="p-4 flex items-center justify-between border-t border-gray-100">
                <div>
                    ${price ? `
                        ...
                        <span class="text-lg font-bold text-primary">${formatCurrency(price)} บ.</span>
                    ` : '<span class="text-sm text-green-600 font-medium">ฟรี ไม่มีค่าใช้จ่าย</span>'}
                </div>
                ${Button({ text: 'รายละเอียด', onClick: onClickAction, className: 'text-sm px-5' })}
            </div>
        </div>
    `;
}
...
/**
 * FeedCardCompact - แสดงโครงการแบบย่อ (สำหรับ sidebar)
 */
